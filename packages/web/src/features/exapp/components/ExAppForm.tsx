...
export const ExAppForm = (props: Props) => {
  ...
  // 機能 ON/OFF（compose profile 連動）。AIアプリの非同期実行は queue profile
  // （elasticmq+worker）依存＝`apps` キーでゲート。閲覧・作成は常時可だが、実行は profile off 時に抑止する。
  const appsEnabled = isUseCaseEnabled('apps');
...
---
  ...
  const onSubmit = async (data: FieldValues) => {
    ...
    // 実行機能（apps＝queue profile）が無効なら起票せず止める（防御。api 側も /exapps/invoke で 503）。
    if (!appsEnabled) {
      setValidationError(
        'AIアプリの実行機能は現在無効です。実行するには deploy の COMPOSE_PROFILES に queue を追加してください。',
      );
      return;
    }
  ...
  return (
    <>
      ...
      <form className='flex flex-col gap-8' onSubmit={handleSubmit(onSubmit)}>
        ...
        {!appsEnabled && (
          <ErrorText>
            ＊AIアプリの実行機能は現在無効です。実行するには deploy の COMPOSE_PROFILES に queue
            を追加してください（アプリの閲覧・作成は引き続き可能です）。
          </ErrorText>
        )}

        <Button
          aria-disabled={requestLoading || !appsEnabled ? true : undefined}
          variant='solid-fill'
          size='lg'
