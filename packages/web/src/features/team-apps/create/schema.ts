import { z } from 'zod';
import { isValidExAppEndpoint } from '@/utils/exAppEndpoint';
...
export const teamAppCreateSchema = z.object({
  name: z.string().min(1, { message: 'AIアプリ名を入力してください' }),
  endpoint: z
    .string()
    .min(1, { message: 'APIエンドポイントを入力してください' })
    .refine(isValidExAppEndpoint, {
      // 公開は https 推奨、ローカルは http://localhost 等を許容（SSRF 制御は api 側）。
