...
import { DIAGRAM_TEMPERATURE, MERMAID_DIAGRAM_TYPES } from '@/features/generate-diagram/constants';
...
export const useDiagram = (id: string) => {
  ...
  const postDiagram = async (content: string, type: MermaidDiagramType | 'AI') => {
    ...
    try {
      ...
      // 4. ダイアグラム生成（mermaid 書式遵守のため低温で生成する）
      await postChat(content, { ignoreHistory: true, temperature: DIAGRAM_TEMPERATURE });
    } catch (error: unknown) {
      setLoading(false);
      throw `${error}`;
    }
