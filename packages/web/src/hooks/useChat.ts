...
export type GenerateOptions = {
  ...
  /** サンプリング温度（任意）。ダイアグラム生成など構造化出力で低温（〜0.2）を指定する用途。 */
  temperature?: number;
};
...
const useChatStore = create<{
  ...
  const createChatIfNotExist = async (id: string, chat?: Chat): Promise<string> => {
    if (chat) {
      return chat.chatId;
    }
  ...
  const addMessageIdsToUnrecordedMessages = (id: string): ToBeRecordedMessage[] => {
      ...
      const newChats = produce(state.chats, (draft) => {
        for (const m of draft[id].messages) {
          if (!m.createdDate) {
            ...
            toBeRecordedMessages.push(JSON.parse(JSON.stringify(m)) as ToBeRecordedMessage);
          }
        }
      });
  ...
  const addChunkToAssistantMessage = (id: string, chunk: string, trace?: string, model?: Model) => {
    set((state) => {
      const newChats = produce(state.chats, (draft) => {
        ...
        const newAssistantMessage: UnrecordedMessage = {
          ...
          content: (oldAssistantMessage.content + chunk).replace(/(<output>|<\/output>)/g, ''),
  ...
  return {
    ...
    updateSystemContext: (id: string, systemContext: string) => {
      set((state) => {
        return {
          chats: produce(state.chats, (draft) => {
            const idx = draft[id].messages.findIndex((m) => m.role === 'system');
            if (idx > -1) {
    ...
    getCurrentSystemContext: (id: string) => {
      ...
      if (chat) {
        const systemMessage = chat.messages.filter((message) => message.role === 'system')[0];
 ...
 * @returns
 */
export const useChat = (id: string, chatId?: string) => {
  const {
    chats,
    loading,
    getModelId,
  ...
  const { data: messagesData, isLoading: isLoadingMessage } = useChatApi().listMessages(chatId);
  const { data: chatData, isLoading: isLoadingChat } = useChatApi().findChatById(chatId);
  ...
  return {
    ...
    pushMessage: (role: Role, content: string) => pushMessage(id, role, content),
