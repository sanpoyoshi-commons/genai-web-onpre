import { FeatureFlags, ModelMetadata } from 'genai-web';
...
export const modelMetadata: Record<string, ModelMetadata> = {
  // ==== Text ===
  // Local (Ollama) — on-premise localization. LLM_BACKEND=ollama のとき api が
  ...
  'mistral:7b': {
    flags: MODEL_FEATURE.TEXT_DOC,
    displayName: '軽量｜Mistral 7B (Ollama, Apache-2.0)',
  },
  'llama3.2:3b': {
    flags: MODEL_FEATURE.TEXT_DOC,
    displayName: '軽量｜Llama 3.2 3B (Ollama)',
  },
  // -- 日本語特化(8B)：Meta Llama 3/3.x（商用可）。Ollama 公式に無く
  //    コミュニティ/HF 配布のため、id は運用者が pull したタグに合わせる。
  //    （ここに無いタグでも displayName=id にフォールバックし crash しない）
  'hf.co/elyza/Llama-3-ELYZA-JP-8B-GGUF': {
    flags: MODEL_FEATURE.TEXT_DOC,
    displayName: '日本語8B｜ELYZA-JP Llama 3 8B (HF/Ollama)',
  },
  'schroneko/llama-3.1-swallow-8b-instruct-v0.1': {
    flags: MODEL_FEATURE.TEXT_DOC,
    displayName: '日本語8B｜Llama 3.1 Swallow 8B (Ollama, community)',
  },
  // -- 大型 --
  mixtral: {
    flags: MODEL_FEATURE.TEXT_DOC,
    displayName: '大型｜Mixtral 8x7B (Ollama, Apache-2.0)',
  },
  'llama3.3:70b': {
    flags: MODEL_FEATURE.TEXT_DOC,
    displayName: '大型｜Llama 3.3 70B (Ollama)',
  },
  // Anthropic
