...
- [概要](#概要)
- [リクエスト形式定義](#リクエスト形式定義)
  - [AIアプリに定義するリクエスト形式](#aiアプリに定義するリクエスト形式)
  - [テキストフィールド](#テキストフィールド)
  - [数値フィールド](#数値フィールド)
  - [テキストエリア](#テキストエリア)
  - [ファイル](#ファイル)
...
genai-web インターフェースは、AI アプリとして外部の REST API を呼び出すことが可能です。
このページは、「チーム管理」メニューからAI アプリを登録する際に「リクエスト形式」に登録する JSON のフォーマットを解説しています。
...
過去の情報をどのように保持するかは AI アプリ側の実装にも依存するため、保持方法については本基盤の Web インターフェースでも複数の方法を用意しています。
...
上記のサンプルを定義すると、genai-web は AI アプリに対して以下のようにリクエストを送出します。
...
genai-web インターフェースは、AI アプリへのリクエストに対するレスポンスとして、以下の形式を期待します。
`outputs` の値であるテキストは、genai-web インターフェースのフロントエンドで処理されます。
