...
AI アプリは、genai-web 向けに定義された API 仕様に従います。

詳細は [AI アプリ API 仕様](./AIアプリAPI仕様.md) を参照してください。

> **ローカル版（onpre）で自作 AI アプリを作る方へ**：`docker compose` で動かすローカル環境で、自作の外部 REST API を
> 作って本基盤に登録し、実際に呼び出すまでの実践ガイド（コピペで動くサンプル・SSRF 設定・トラブルシュート込み）は
> **[AI アプリ ローカル開発ガイド](./AIアプリローカル開発ガイド.md)** を参照してください。
...
**図: genai-web での個別の AI アプリを作成（登録）する画面の後半部分** (開発中のもの)
